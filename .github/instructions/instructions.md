...
- `src/router.tsx` must export `getRouter` (not `createRouter`) — TanStack Start's server-core calls `entries.routerEntry.getRouter()` explicitly. Exporting any other name silently breaks SSR with a 500.
- `ScrollRestoration` component is deprecated — use `scrollRestoration: true` in `getRouter`'s `createRouter` call instead. Do not import or render `<ScrollRestoration />`.
- `routeTree.gen.ts` is auto-generated by the TanStack Router Vite plugin on dev server start. It must exist before the first request. If missing: `bunx @tanstack/router-cli generate` from `apps/web/`.
